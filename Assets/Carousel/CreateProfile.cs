		...
		for(int i = 0; i < introImages.Length; i++){
			...
			if(side == "right") {
				if(i == pageCount-1) {
					...
					Color temp = introImages[i].GetComponent<Text>().color;
					introImages[i].GetComponent<Text>().color = new Color(temp.r,temp.g,temp.b,1);
				} else {
					introImages[i].localScale = Vector3.Lerp(introImages[i].localScale,new Vector3(0.7f,0.7f,0.7f),Time.deltaTime*5);
					Color temp = introImages[i].GetComponent<Text>().color;
					introImages[i].GetComponent<Text>().color = new Color(temp.r,temp.g,temp.b,0.5f);
				}
			} else {
				if(i == pageCount) {
					introImages[i].localScale = Vector3.Lerp(introImages[i].localScale,new Vector3(1.2f,1.2f,1.2f),Time.deltaTime*5);
